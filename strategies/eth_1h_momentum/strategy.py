"""LightGBM 이진 분류 전략 (롱 전용) — ETH/1h 모멘텀.
...
class LGBMClassifierStrategy(BaseStrategy):
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.models_dir = config.get(
            "models_dir", "strategies/eth_1h_momentum/models"
        )
    ...
    def _load_model(self) -> lgb.Booster:
        ...
        model_path = self.config.get(
            "model_path",
            "strategies/eth_1h_momentum/models/latest.txt",
        )
    ...
    def _load_feature_names(self) -> list[str]:
        ...
        path = self.config.get(
            "feature_names_path",
            "strategies/eth_1h_momentum/models/feature_names.json",
        )
