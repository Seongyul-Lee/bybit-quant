...
Funding Arb 전략은 별도 실행 경로(ArbExecutor, Buy & Hold).
Bybit Hedge Mode 활성화 — 동일 심볼에서 v1 전략과 funding_arb가 동시 운용 가능.

## 커맨드
```bash
.venv/Scripts/activate && pip install -r requirements.txt  # 환경 설정
python -m pytest tests/ -v                                  # 전체 테스트
python -m pytest tests/test_common_features.py -v           # 단일 파일 테스트
python -m pytest tests/ -k "test_signal" -v                 # 키워드 필터 테스트
python train_lgbm.py --strategy btc_1h_momentum --no-optuna # 모델 학습 (고정 파라미터)
python train_lgbm.py --strategy eth_1h_momentum --optuna-trials 100  # Optuna 튜닝
python train_lgbm.py --strategy btc_1h_momentum --load-params        # 기존 파라미터 로드
python backtest.py --strategy btc_1h_momentum               # 백테스트
python oos_validation.py --strategy btc_1h_momentum         # OOS 검증
python oos_validation.py --strategy btc_1h_momentum --json  # JSON 구조화 출력
...
실거래: Collector → Processor → 각 Strategy.generate_signal(df) → PortfolioManager.allocate()
        ...
        (funding_arb는 별도: ArbExecutor로 spot+perp 동시 실행, PortfolioManager 미경유)
...
| 모듈 | 클래스 | 역할 |
|------|--------|------|
| `src/strategies/base.py` | `BaseStrategy` | 추상 기반 — `generate_signal(df)→(int,float)` |
| `src/portfolio/manager.py` | `PortfolioManager` | 멀티 전략 자본 배분 |
| `src/portfolio/risk.py` | `PortfolioRiskManager` | 포트폴리오 MDD 방어 (4단계) |
| `src/portfolio/virtual_position.py` | `VirtualPositionTracker` | 동일 심볼 멀티 전략 포지션 추적 |
| `src/risk/manager.py` | `RiskManager` | 전략별 SL/TP, CircuitBreaker |
| `src/risk/arb_monitor.py` | `ArbRiskMonitor` | 펀딩 아비트리지 전용 리스크 |
| `src/execution/executor.py` | `OrderExecutor` | 멱등 주문 실행, trade_log.csv |
| `src/execution/arb_executor.py` | `ArbExecutor` | spot+perp 동시 실행 |
| `src/data/collector.py` | `BybitDataCollector` | OHLCV/펀딩비/OI 수집 |
| `src/data/processor.py` | `DataProcessor` | 기술적 지표 계산 |
| `strategies/_common/features.py` | `FeatureEngine` | ML 피처 65개 생성 (symbol별 펀딩비 경로) |
| `strategies/_common/labeler.py` | `TripleBarrierLabeler` | ATR 기반 2클래스 라벨링 |
| `strategies/_common/trainer.py` | `WalkForwardTrainer` | Expanding/Sliding Walk-Forward + Optuna |
...
data/raw/bybit/      → {SYMBOL}USDT/{timeframe}/ 원본 OHLCV + funding_rate.parquet + open_interest_1h.parquet
...
strategies/funding_arb/           → strategy.py, config.yaml (Buy & Hold, ArbExecutor 경유)
...
- **모델 파일 CRLF 주의**: git checkout으로 모델(.txt) 복원 시 LF→CRLF 변환으로 LightGBM 모델 손상됨. `sed -i 's/\r$//'`로 수정 필요.
...
- **라벨러 선택**: config.yaml `labeler_type`으로 동적 선택 (기본: triple_barrier, mean_reversion 등)
- **재학습 비결정성**: 동일 데이터/파라미터/seed로도 Windows에서 결과가 달라짐. 커밋된 모델 파일을 직접 보존할 것.
...
MDD 방어 4단계: -3%→75%, -5%→50%, -7%→25%, -10%→전면 중단.
