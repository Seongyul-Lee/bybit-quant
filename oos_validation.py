...
def run_oos_validation():
    ...
    # 성공 기준 판단
    is_result = results.get("In-Sample", {})
    pv_result = results.get("Post-Validation", {})

    if pv_result.get("trades", 0) == 0:
        print("FAIL: Post-Validation 구간에 거래가 없습니다.")
        return results
    ...
    print(f"\n  IS PF: {is_pf:.2f} → Post-Val PF: {pv_pf:.2f} (하락률: {pf_drop:.1f}%)")
